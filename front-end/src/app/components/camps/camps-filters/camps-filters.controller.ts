...
import { getModelFilters } from '../camps.selectors';

export class CampsFiltersComponentController {
  ...
  private _mapDispatchToThis = dispatch => {
    return this.CampsService.mapDispatchToThisIndex()(dispatch);
  };

  private _mapStateToThis = (state) => {
    const filters = getModelFilters(state);
    return this.CampsService.mapStateToThisIndex({
